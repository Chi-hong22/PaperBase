...
PaperBase 是专为学术研究设计的论文知识库脚手架，解决传统文献管理工具（Zotero、Mendeley）在 AI 时代的核心痛点：**无法将论文转化为机器可理解的结构化知识**。

当研究者需要从数百篇论文中提取关键概念、追溯方法演进、构建领域知识图谱时，现有工具只能提供 PDF 文件和元数据，而 PaperBase 提供的是：
- 规范化的 Markdown 文档（包含完整语义结构）
- 可重建的知识图谱投影（论文关系网络）
- 幂等的处理流程（可中断、可恢复、可追溯）
- AI Agent 友好的接口（CLI + 结构化输出）

**适用场景**：
- 构建个人学术知识库，支持深度检索和语义分析
- 为 AI Agent 提供可靠的论文数据源（LLM 应用、研究助手）
- 团队协作的文献管理（基于 Git 版本控制）
- 领域知识图谱构建（方法论演进、引用网络分析）

**核心价值**：不是替代 Zotero，而是将论文从"文件"升级为"知识资产" —— 从不可查询的 PDF 变为可检索、可图谱化、可编程的结构化数据。

---

## 核心特性

PaperBase 通过以下设计理念实现论文知识化：

- **Canonical Markdown 是唯一真相源** - 所有数据投影（图谱、索引、分块）均可从规范化 Markdown 重建，避免数据不一致
- **幂等状态机管理** - 论文处理流程（下载 → 转换 → 规范化 → 图谱化）可中断、可恢复、可追溯，适合批量处理
- **内容寻址存储** - PDF 和元数据分离存储（基于 SHA256），避免重复存储，同一 PDF 可被多篇论文引用
- **工具无关设计** - 既可用于 AI Agent 自动化工作流，也可用于传统命令行脚本，输出标准化 Markdown 和 JSON
...
**为什么不直接用 Zotero？**
- Zotero 适合文献管理，但不适合作为 AI Agent 的知识库：难以图谱化、检索粒度粗、schema 不可控
- PaperBase 与 Zotero 互补：可通过 MCP 从 Zotero 导入论文，但以结构化 Markdown 作为存储格式
...
**为什么分离 PDF 存储？**
- 内容寻址（SHA256）避免重复存储，同一 PDF 可被多篇论文元数据引用
