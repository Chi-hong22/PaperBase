...
- **调用方式**: CLI (`graphify extract <path> --backend openai --model <model>`)
- **职责**: 知识图谱构建、语义分析
...
library/papers/
  ├── p_2ddac761b162/
  │   ├── paper.md           # Canonical Markdown（唯一真相源）
...
**验证结果：**
- ✅ graphify 能正确扫描立体结构（29 nodes, 28 edges）
- ✅ 立体结构优于扁平化（更好的文件组织 + 更多图谱节点）
- ✅ 修复 `.gitignore` 后完全可行
