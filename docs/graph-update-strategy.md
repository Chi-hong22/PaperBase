...
预检发现 `NEEDS_REVIEW` 时，PaperBase 会在耗时建图前停止并保留旧图；只修复对应论文的摄入/Canonical 内容后重跑，不在 Graphify 阶段直接读取 PDF 或原始 URL。`BLOCKED` 论文不进入增量候选，也应由 `.graphifyignore` 显式排除，直到阻塞原因被人工解除。
...
- **阻塞论文不建图**：`BLOCKED` 不属于可重试候选；增量检测跳过它，Graphify 扫描规则也必须排除对应 Canonical。
...
PaperBase 故意不让 Git 跟踪真实论文内容，但 Graphify 仍需读取本地 Canonical。两套 ignore 规则各司其职：

```text
.gitignore                         -> 排除 library/papers/p_*.md，避免论文进入提交和远程历史
library/papers/.graphifyignore    -> !p_*.md 重新纳入本地 Canonical，并排除 BLOCKED 文件
```

不要用 `git add -f` 解决 Graphify 扫描问题；应检查 `.graphifyignore` 的最后匹配规则。
...
**症状**：修改了 `library/papers/p_<storage_id>.md` 但增量更新跳过
